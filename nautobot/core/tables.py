            ...
            for column in self.columns:
                ...
                for field_name in accessor.split(accessor.SEPARATOR):
                    ...
                    if isinstance(field, ForeignKey) and not prefetch_path:
                        ...
                        # ForeignKey segments already walked (as in `interface__ip_addresses`) become
                        # the prefix of the prefetch path; they stay in select_fields too, and the
                        # prefetch traversal reuses the objects the join already brought in. Without
                        # this the to-many hop was dropped and became a per-row read.
...
class ContentTypesColumn(django_tables2.ManyToManyColumn):
    ...
    def filter(self, qs):
        ...
        Sorted in Python rather than with `qs.order_by()`: `order_by()` clones the queryset, which
        discards the prefetch cache `BaseTable` sets up for `content_types` and puts back one query
        per rendered row. A content-type list is a handful of rows, so sorting it in Python is free.
        """
        if self.sort_items:
