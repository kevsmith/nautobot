...
How every number in [`perf/report.md`](report.md) was taken, what the instruments are, and the
working behind each finding. This file exists so the report can be short: it answers "why should
I believe that", which is a different question from "should we adopt this".

<!--GEN:provenance-->

## The instruments, and a warning about the word "tier"

Two unrelated taxonomies in this project share the word. **Risk tiers** — `A`,
`B1`, `B2`, `C`, `M0`–`M2` — price what a change costs to adopt, and are what
the report's Tiers table describes. **Instrument tiers** — Tier 1, Tier 1W,
Tier 2 — are harness scripts, and the numbering records the order they were
built rather than any hierarchy of authority. The names below are what each one
measures; prefer them.
...
The numbering also implies something that stopped being true. Tier 2 was built
as the slow, noisy confirmation of a Tier 1 result, and its own docstring still
says so; at concurrency 1 it holds a 1.6% median spread and agrees with
in-process timing to 1.08×, which is not a confirmation step but a peer. The
scripts keep their names because dozens of finding records cite them as
historical fact, and renaming the committed baseline files would break the
provenance those records depend on. A rename is on the queue.
...
host. No before/after column: these are references; they replace a Tier 2 instrument that
...
**Query counts are bounded by page size rather than dataset size.** At 11× the data the counts
...
change's real effect, and no cumulative figure is affected. Recorded because an A/B result
depends on which *before* file it was compared against.
...
The precision matters less than the corroboration. The two independent instruments now agree:
HTTP at concurrency 1 runs a median **1.08×** in-process across fourteen shared endpoints,
range 1.04–1.19×, which is what HTTP parsing, WSGI and response transfer should cost on top of
identical Django work. Under the old configuration they
disagreed by 4–6× and nothing could say which was right.

Before that they disagreed in a way that was physically impossible. `ui.device.interfaces`
reported a 66 ms HTTP p95 against a 607 ms in-process median. An API token authenticates DRF
only, so every UI endpoint had been answering **403** with a 299 KB permission-denied page that
renders in ~50 ms. cassowary counts an answered request as a
...
- **No stock-vs-branch aggregate read-path wall clock has been taken.** Round one and round two
  ran on different hosts, so the only cumulative read-path figure that survives the host change
  is a query count. The measurement is affordable on the current host and has not been run.
- **The demo-instance gap needs re-measuring with one instrument.** The 4.7× figure in the
  report compares in-process medians here against HTTPS-to-uwsgi there. Both sides can now be
  taken at concurrency 1 under uwsgi.

## Caveats

- **Test coverage is complete for the tree these numbers describe.** A whole-suite run —
  **17,505 tests in 8,151 s** — passed against the exact tree every measurement below was taken
  against: `OK (skipped=663, expected failures=1)`, zero failures, zero errors, product tree
...
- **The new request-scoped state is now tested, and the third-party coupling fails loudly.**
  ...
  is identical, so an upgrade that changes `render()` semantics breaks a test rather than
  producing wrong cell content — and writing it surfaced a coupling nobody had recorded: the
  reimplementation depends on `kwargs["bound_row"]` reaching `get_context_data()`, not only on
  that hook existing, so there is now an explicit test for the kwarg contract.
  ...
  to be read as a set.
- **Absolute wall-clock figures are not comparable across machines or datasets.** Round one ran
  on Apple Silicon, round two on an Intel i5-8259U with turbo disabled — 4–6× slower and far
  more precise. Relative comparisons hold; absolute ones do not.
...
- **The write screen measures a floor, and the factor is now bounded for most of the surface.**
  ...
  with populated payloads at all, including `dcim.cable`, `dcim.interface` and `dcim.powerfeed`,
  because a model with expensive optional relations is both the most understated and the hardest
  to build a payload for. `dcim.cable` is quantified by another route — finding 40 measured a
  connected cable at ~220 queries against the floor's 44.9, which is 4.9× and consistent with the
  top of the measured range. See finding 47.
