...
#
#   perf/apply_arm.sh <git-ref> <label>
#
...
psql_() { perf/dc.sh exec -T db psql -U nautobot "$@" </dev/null; }

"$REPO"/perf/arm_control.sh reset   || exit 1
"$REPO"/perf/arm_control.sh arm "$REF" || exit 1
...
NBHASH=$("$REPO"/perf/arm_control.sh hash)
