...
- **Whole-workflow apply, 9,972 objects**: both arms from a database reset to the same empty template, arms alternated, trees proved different by content hash, and each arm ended with identical row counts -- 536 devices, 1,616 interfaces, 1,648 cables -- so the two runs did the same work. **Of the 733 seconds saved, 4.5 are database execution**.
