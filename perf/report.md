...
>
> Sources: tree `959290feb` with 7 dirty path(s); read-path queries from `perf/baselines/uwsgi-tier1-current.json` against `large-tier1-baseline.json`; write path from `uwsgi-tier1w-baseline.json`; in-process wall clock from `uwsgi-bench-r{1,2,3}.json`; HTTP wall clock from `uwsgi-tier2-c1-r{1,2,3}.json`, concurrency 1, median.

## Optimizations identified

32 proposed changes to Nautobot: **21** accepted · **11** rejected. Every one is listed, including the ones that did not work -- a rejected optimization is a measurement of what an option costs, and deleting it would invite the next person to try it again.
...
9 of the 41 experiments changed the harness rather than Nautobot: new instruments, and measurements of the instruments themselves. They are recorded to the same standard because a measurement is only as good as the thing that took it.
