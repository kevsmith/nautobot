...
#
#   perf/scripts/run_experiment.sh <name> [--writes]
#
...
# perf/scripts/record_expected_counts.sh. Override with PERF_EXPECTED for another tier.
EXPECTED="${PERF_EXPECTED:-$(cat "$ROOT/perf/baselines/expected-counts.txt" 2>/dev/null || echo "")}"
if [ -z "$EXPECTED" ]; then
  echo "!! no expected counts recorded; run perf/scripts/record_expected_counts.sh first"
  exit 3
fi
ACTUAL=$(cat <<'PY' | perf/scripts/dc.sh exec -T nautobot nautobot-server shell 2>/dev/null | tail -1
...
if [ "$ACTUAL" != "$EXPECTED" ]; then
  ...
  echo "   restore with: perf/scripts/restore_snapshot.sh   (comparison is invalid until then)"
  exit 3
fi

echo "== Tier 1 (read path) =="
perf/scripts/dc.sh exec -T nautobot python /source/perf/scripts/tier1_queries.py \
  --out "/source/perf/results/tier1-${NAME}.json" 2>&1 | tail -1

if [ "$WRITES" = "--writes" ]; then
  echo "== Tier 1W (write path) =="
  perf/scripts/dc.sh exec -T nautobot python /source/perf/scripts/tier1w_writes.py \
    --out "/source/perf/results/tier1w-${NAME}.json" 2>&1 | grep -vE "^\s|^\d{2}:" | tail -15
fi

echo "== verdict =="
python3 perf/scripts/compare.py \
