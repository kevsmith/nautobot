...
#
#   perf/scripts/sync.sh [--full] [--restart]
#
...
for arg in "$@"; do
  case "$arg" in
    --full)    FULL=1 ;;
    --restart) RESTART=1 ;;
    *) echo "usage: perf/scripts/sync.sh [--full] [--restart]" >&2; exit 2 ;;
  esac
done
...
if [ "$RESTART" -eq 1 ]; then
  ...
  ssh -o BatchMode=yes "$PERF_HOST" "cd '$PERF_PATH' && perf/scripts/dc.sh restart nautobot" >/dev/null
...
for i in $(seq 1 60); do
  status="$(perf/scripts/dc.sh ps --format '{{.Name}} {{.Status}}' 2>/dev/null | grep -c 'nautobot-1.*healthy')"
  [ "$status" = "1" ] && break
  sleep 2
done
