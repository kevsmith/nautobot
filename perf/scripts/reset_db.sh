...
#
#   perf/scripts/reset_db.sh --build    (re)build the template from the current database
#   perf/scripts/reset_db.sh            clone the template over the live database
#
# Why this exists. perf/scripts/restore_snapshot.sh takes ~49 seconds, and four of its
...
psql_postgres() { perf/scripts/dc.sh exec -T db psql -U nautobot -d postgres -q "$@"; }
...
usage() { echo "usage: perf/scripts/reset_db.sh [--build]" >&2; exit 2; }
...
if [ -z "$STORED" ]; then
  ...
  echo "run: perf/scripts/restore_snapshot.sh && perf/scripts/reset_db.sh --build" >&2
  exit 1
fi
