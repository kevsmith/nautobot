...
#
#   perf/scripts/probe_clone_warmup.sh [rounds] [requests-per-arm]
#
# perf/scripts/reset_db.sh returns the database to baseline in ~1.3s by copying the
...
for r in $(seq 1 "$ROUNDS"); do
  perf/scripts/reset_db.sh || { echo "clone failed in round $r" >&2; exit 1; }
