...
followed by ``perf/scripts/reset_db.sh``, which is what finding 33 made affordable; it
...
#
# `--isolation reset` cannot shell out to perf/scripts/reset_db.sh, because this script
...
def check_template(fatal=True):
    ...
    if fatal:
        ...
        raise SystemExit(f"{detail}\nrun: perf/scripts/restore_snapshot.sh && perf/scripts/reset_db.sh --build")
    return False, stored
...
def main():
    ...
    ap.add_argument(
        ...
        default="rollback",
        help="rollback: in-process transaction, undone (default). reset: commit, then perf/scripts/reset_db.sh "
