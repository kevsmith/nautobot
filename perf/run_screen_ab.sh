...
#
#   perf/run_screen_ab.sh <reads|writes> <stock-ref> <branch-ref> [--rounds N]
#
# Runs ON the measurement host, under perf/measure.sh, which takes the lock and
...
#
#   Each arm proves its own tree. perf/arm_control.sh prints the content hash of
...
run_arm() {
  ...
  echo "== arm $label ($ref) =="
  local armout
  armout="$(perf/arm_control.sh arm "$ref")" || return 1
  echo "$armout"
  local hash
  hash="$(printf '%s\n' "$armout" | sed -n 's/.*nautobot-hash \([0-9a-f]*\).*/\1/p')"
  [ -n "$hash" ] || { echo "no tree hash from arm swap" >&2; return 1; }
  echo "$hash" > "${out%.json}.treehash"
  perf/dc.sh exec -T nautobot python "/source/perf/$SCRIPT" --out "/source/perf/results/$(basename "$out")" \
    || return 1
}
